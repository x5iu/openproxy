...
- `README.md`: user-facing config and deployment documentation, including `forward` provider transparency/auth semantics and the global HTTP trailer-ignore policy across HTTP/1.1 and HTTP/2.
...
- for protocol behavior changes, look at `test_http.py`, `test_https.py`, `test_h1_trailers.py`, `test_h2_upstream.py`, `test_h2_large_body.py`, `test_h2_h1_fallback.py`, `test_h2_h1_fallback_framing.py`, `test_connect_tunnel.py`, and `test_websocket.py`; pair H2→H1 framing/trailer changes with Rust unit tests in `src/worker/mod.rs` and `src/http/mod.rs` that inspect stripped trailer metadata, rebuilt framing headers, and forwarded body bytes
